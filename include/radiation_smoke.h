...
#ifndef RADIATION_SMOKE_H
#define RADIATION_SMOKE_H
...
class Radiation_smoke : public Radiation<TF>
{
    public:
        Radiation_smoke(Master&, Grid<TF>&, Fields<TF>&, Input&);
        virtual ~Radiation_smoke();
        ...
        void get_radiation_field(Field3d<TF>&, const std::string&, Thermo<TF>&, Timeloop<TF>&)
            { throw std::runtime_error("\"get_radiation_field()\" is not implemented in radiation_smoke"); }
        std::vector<TF>& get_surface_radiation(const std::string&)
            { throw std::runtime_error("\"get_surface_radiation()\" is not implemented in radiation_smoke"); }
        ...
        TF* get_surface_radiation_g(const std::string&)
            { throw std::runtime_error("\"get_surface_radiation_g()\" is not implemented in radiation_smoke"); }
        void prepare_device() {}
        void clear_device() {}
        void forward_device() {}
